...
- (void)loadView
{
    ...
    navigationBar.tintColor = [UIColor colorWithRed:0.27 green:0.34 blue:0.39 alpha:1.0];
...
- (void)viewDidAppear:(BOOL)animated
{
  ...
  if (self.interfaceOrientation == UIInterfaceOrientationLandscapeLeft || 
    ...
    navigationBar.frame = CGRectMake(0, navigationBar.frame.origin.y, 480, 32);
    webViewContainer.frame = CGRectMake(0, navigationBar.frame.size.height, 480, 
                               300 - navigationBar.frame.size.height);
  }    
  else {
    //[[UIApplication sharedApplication] setStatusBarHidden:NO animated:YES];
    navigationBar.frame = CGRectMake(0, navigationBar.frame.origin.y, 320, 44);
    webViewContainer.frame = CGRectMake(0, navigationBar.frame.size.height, 320, 
                               460 - navigationBar.frame.size.height);
  }
}
...
- (void)pageLoadFailed
{
    ...
    // display an error
//    UIAlertView * v = [[UIAlertView alloc] initWithTitle:@"Network Error" message:@"CraigsFish could not connect to the community server!" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Try Again", nil];
//    [v show];
//    [v release];
  
  // TODO: use message here instead of default
  [[StatusBarNotifier sharedNotifier]
   flashLoading:@"" deferred:
   [DKDeferred fail:[NSError errorWithDomain:@"" code:0 userInfo:EMPTY_DICT]]];
