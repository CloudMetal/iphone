...
{ 
    ...
    sqlite3_stmt *stmt;
    char s[140];
    sqlite3_snprintf(140, s, "SELECT value,timeout,flags FROM dkcache WHERE "
    ...
    @synchronized(self) {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            id value = (f == 1) ? d : (f == 2) ? [UIImage imageWithData:d] :
...
timeout:(NSNumber *)timeout arg:(id)arg 
{
    if (! [[value class] canBeStoredInCache] && 
        ! [value isKindOfClass:[UIImage class]]) return nil;
    ...
    int f = [value isKindOfClass:[NSData class]] ? 1 : 
            [value isKindOfClass:[UIImage class]] ? 2 : 0;
    NSData *d = (f == 1) ? value : (f == 2) ? UIImagePNGRepresentation(value) : 
...
cullFrequency:(int)_cullFrequency 
{
    ...
    sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS dkcache (key UNSIGNED INTEGER "
                 ...
                 "DEFAULT 0, stamp DATETIME DEFAULT CURRENT_TIMESTAMP)", NULL, 
                 NULL, NULL);
